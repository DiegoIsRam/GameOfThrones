...
export const getAllHouses = () => dispatch => {
    ...
    .then(housesData => dispatch({type: GET_ALL_HOUSES, payload:  housesData} ))
};
...
export const getHouse = (id) => dispatch => {
    ...
    .then(response => response.json())
    .then(houseData => dispatch({type: GET_HOUSE, payload: houseData}))
};
...
export const createHouse = (houseDataToCreate) =>{
    id++;
    return {type: CREATE_HOUSE, payload: {...houseDataToCreate,id}}
}

// Desde el componente ejecutamos la action creator, pasandole como argumento el id de la house que queremos eliminar.

export const deleteHouse = (idHouseToDelete) => {
    return {type: DELETE_HOUSE, payload: idHouseToDelete}
};
